...
「吹き出し素材」タブでは、楕円・角丸・ギザギザの吹き出しを透明PNGとして作成できます。吹き出しをOFFにすると文字だけを出力でき、塗りの不透明度は0〜100%で調整できます。しっぽは4方向Presetから選び、Previewの青いHandleをドラッグして先端位置を調整できます。Handleは操作専用で、保存画像には含まれません。保存後は実際に保存された画像または保存先をすぐ開けます。

起動時のウィンドウは、カーソルがある画面（取得できない場合はPrimary Screen）のタスクバー等を除いた利用可能領域へ自動的に収まります。
