...
保存先は各画面に表示され、必要に応じて変更できます。Windowsで使えないファイル名の文字は安全な文字へ置き換え、同名ファイルがある場合は `_2`、`_3` のように連番を付けます。擬音素材と吹き出し素材は保存直後に「画像を開く」「保存先を開く」を選べます。その他の処理は結果一覧または保存先から確認できます。
